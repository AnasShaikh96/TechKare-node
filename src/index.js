...
const mySql				= require("." + "/" + "libs" + "/" + "mySqlSequelize.js").mySqlSequelize;
...
module.exports.index = class index{
	...
	*/
	...
	loadConfig(configType,configPath=null){
		this._messages = require(__dirname + "/" + "configs" + "/" + "messages.js").messages;
		if(configType==0){
			this._config['app'] = require(__dirname + "/" + "configs" + "/" +"app");
			this._config['database'] = require(__dirname + "/" + "configs" + "/" + "database");
			
			if(fs.existsSync(__dirname + "/" + this._package.name + "/" +  "/crons/scheduler.js")){
				 this._cron = new (require(__dirname + "/" + this._package.name + "/" +  "/crons/scheduler.js").crons)(this._config['database']);
			}else{
	...
	*/
	...
	routes(){
		...
		new (require(__dirname + "/" + this._package.name + "/" + "routes" + "/" + "web_routes.js").web_routes)(app).load();
		new (require(__dirname + "/" + this._package.name + "/" + "routes" + "/" + "app_routes.js").app_routes)(app).load();
		new (require(__dirname + "/" + this._package.name + "/" + "routes" + "/" + "error_routes.js").error_routes)(app).load();
		// app.use(this.endDb);
	}
...
//		res.setHeader('Access-Control-Allow-Headers', 'X-Requested-With,contenttype'); // If needed
	...
	expreesConfiguration(){
		...
		if(this._config.hasOwnProperty("app")){
			...
			const session_file_options = {
				path: __dirname+(!this._config.app._sessionPath) ? "../sessions" : this._config.app._sessionPath,
				encrypt:true
			}

			this._session = Session({
				store:new FileStore(session_file_options),
				secret: (!this._config.app.sessionSecret) ? "JJHUYNSKJNSJKCNSDJKNJKNJKDCKSNCKSD" : this._config.app.sessionSecret,
				resave: true,
				saveUninitialized: true,
				rolling:true,
				name:(!this._config.app.sessionName) ? this._package.name : this._package.name,
				cookie:{
					secure:false,
					maxAge:86400000
				}
			...
			var p = require(__dirname + "/" + "libs" + "/" + "passport.js").passport(passport,app,this._config.database);
	...
	*/
	...
	startServer(){
		if(fs.existsSync(__dirname + "/" + this._package.name + "/" + "/schema/schemas.js")){
			var db = new mySql();
			...
			db.createConnection(function(r,err,success){
				if(err){
					console.log(err);
					//res.json({success:0,message:"Database Connection Error"});
					//r.endDb(req,res);
					...
					console.log("Database connected");
					new (require(__dirname + "/" + r._package.name + "/" + "/schema/schemas.js").schemas)(success.con).setUp();
				}
			}.bind(null,this));
		}else{
		...
		if(this.hasOwnProperty("_config")){
			if(Object.keys(this._config).length>0){
				...
				this._server.on("listening",function(r){
	...
	*/   
    ...
    conditionalCSRF(req, res, next) {
        ...
        var path = req.path;
        var needCSRF = true;
        if(path.indexOf("api") >= 0 || req.path.indexOf("/admin/products/bulk-upload")!=-1){
            needCSRF=false;
        }
