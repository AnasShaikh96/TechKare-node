...
const helper = require(".." +
  ...
  "helpers" +
  "/" +
  "common_helper.js").common_helper;

/**
 * Export Base Model Class
 * @memberof com.techkare.models
 * @module base_model
 * @see com.techkare.models.base_model
 */
...
module.exports.base_model = class base_model {
  constructor(name, db) {
    if (name) {
      ...
      ContollerFile =
        ...
        "daos" +
        "/" +
        ContollerFile.replace("model", "dao");
      var DTOFile =
        __dirname +
        "/" +
        ".." +
        "/" +
        "dtos" +
        tempName.replace("model", "dto");
      this.dao = new (require(ContollerFile)[
        path.basename(ContollerFile).replace(".js", "")
      ])(db);
   ...
   * @public
   * @memberof com.techkare.models.base_model
   * @function create
  ...
  getRecord(filterObj, callback, query = "", join_conditions = "") {
    ...
    console.log(query);
    if (
      typeof filterObj.page !== typeof undefined &&
      typeof filterObj.limit !== typeof undefined
    ) {
      var resultObj = {};
      var countQuery =
        "select count({{tableName}}.id) as recCount from {{tableName}} " +
        filterQuery;
      this.dao.select(
        countQuery,
        function (r, error, success) {
          if (error) {
            ...
            query +=
              " limit " +
              (parseInt(filterObj.page) - 1) * parseInt(filterObj.limit) +
              "," +
              parseInt(filterObj.limit);
