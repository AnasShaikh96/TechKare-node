 ...
 * @see com.techkare.helpers.common_helper
...
module.exports.common_helper = class common_helper extends baseHelper {
   ...
   *  @public
   *  @memberof com.techkare.helpers.common_helper
  ...
  createFilter(filter, allowdeleted = true, handler = "and") {
    ...
    if (typeof filter !== typeof undefined && filter != null) {
      ...
      query +=
        (filter.length > 0 ? " and " : "") +
        (allowdeleted ? "deleted_at is NULL" : "1=1");
    } else {
      query += "where ";
  ...
  formatValidationError(validationModel, req = "") {
    ...
    var errorMessage = {};
    if (
      validationModel.hasOwnProperty("error") &&
      validationModel.error != null
    ) {
      ...
      for (var i in errorMessages) {
        if (!errorMessage.hasOwnProperty(errorMessages[i].path)) {
          ...
          if (req) {
            req.oldInput.setError(
              errorMessages[i].path,
